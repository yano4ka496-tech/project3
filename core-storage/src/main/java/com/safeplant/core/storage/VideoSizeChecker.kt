...
class VideoSizeChecker(private val context: Context) {
    companion object {
        // Максимальный разрешенный размер видео: 150 МБ (переделано в байтах)
        const val MAX_SIZE_BYTES = 150 * 1024 * 1024L
    }
     ...
     */
    suspend fun checkTotalSize(): Result<Long> =
        withContext(Dispatchers.IO) {
            val assetManager = context.assets
            try {
                val videoFiles = assetManager.list("videos") ?: return@withContext Result.success(0L)
                var totalSize = 0L
     ...
     */
    suspend fun getVideoFiles(): List<String> =
        withContext(Dispatchers.IO) {
            val assetManager = context.assets
            try {
                val videoFiles = assetManager.list("videos") ?: return@withContext emptyList()
