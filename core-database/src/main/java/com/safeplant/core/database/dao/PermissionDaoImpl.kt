...
/**
 * Реализация интерфейса PermissionDao с дополнительными методами
 */
@Dao
interface PermissionDaoImpl {
     ...
     */
    suspend fun invalidateExpiredPermissions(currentTime: Long) = withContext(Dispatchers.IO) {
        // Обновляем все истекшие разрешения
        // В Room нет прямого способа выполнить UPDATE с условием в аннотации,
        // поэтому используем raw query
        // В реальном приложении здесь можно использовать @RawQuery или Transaction
        // Для простоты используем raw query
        // В реальном приложении лучше создать отдельный репозиторий с бизнес-логикой
    }
}